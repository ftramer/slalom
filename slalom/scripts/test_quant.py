...
from slalom.quant import Conv2DQ, DenseQ, DepthwiseConv2D, DepthwiseConv2DQ
    ...
    for i, layer in enumerate(model.layers):
        if isinstance(layer, Conv2D) or isinstance(layer, DepthwiseConv2D):
            ...
            if isinstance(layer, Conv2D):
                ...
                layer_map[layer] = DepthwiseConv2DQ.from_config(conf)
...
def slow_verify_linear_comps(linear_outputs):
    for i in range(len(linear_outputs)):
        ...
        if len(X.shape) == 4:
            ...
            raise AttributeError("linear input has shape {}".format(X.shape))
